                    ...
                    filtered.map((p) => {
                        ...
                        return (
                            <div
                                ...
                                className={`group flex justify-between items-center p-3 rounded-xl cursor-pointer transition-all border backdrop-blur-sm shadow-sm ${isSelected
                            ...
                            >
                                <div className="flex items-center gap-3 overflow-hidden">
                                    {/* Author Avatar */}
                                    <div className={`w-10 h-10 shrink-0 rounded-full flex items-center justify-center text-white font-bold text-sm bg-gradient-to-br ${isSelected ? 'from-blue-500 to-indigo-600 shadow-md' : 'from-gray-500 to-gray-600 dark:from-gray-600 dark:to-gray-700'}`}>
                                        {p.author.username.charAt(0).toUpperCase()}
                                    </div>

                                    <div className="flex flex-col overflow-hidden">
                                        <h6 className={`font-semibold text-sm truncate ${isSelected ? 'text-blue-800 dark:text-blue-200' : 'text-gray-900 dark:text-gray-100'}`}>
                                            {p.author.username}
                                        </h6>
                                        <p className={`text-xs font-medium truncate ${isSelected ? 'text-blue-600 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400'}`}>
