...
const Contributors: React.FC = () => {
  ...
  const contributorsLeaderboard = useMemo(() => {
    ...
    paragraphs.forEach((item) => {
      ...
      if (!grouped[username]) {
        grouped[username] = { username, count: 0 };
      }
    ...
    result.sort((a, b) => {
      ...
      if (selectedFilter.name === "A-Z") return a.username.localeCompare(b.username);
      return 0;
    });
  ...
  return (
    <div className="flex flex-col gap-6 w-full mx-auto font-mono pb-10">
            ...
            >
              {filterOptions.map((option) => (
                ...
                >
                  {({ selected }) => (
                    <>
                      <span className={clsx("block truncate", selected ? "font-bold text-white" : "font-medium")}>
                        {option.name}
                      </span>
                      {selected && (
